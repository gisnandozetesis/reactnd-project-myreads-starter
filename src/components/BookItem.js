...
class BookItem extends Component {
  ...
  handleBookURL(book) {
    if (book.imageLinks) return `url(${book.imageLinks.thumbnail})`;
    else return `url(${NoImage})`;
  }
        ...
        <div className="book-top">
          <div
            ...
            style={{
              width: 128,
              height: 193,
              backgroundImage: this.handleBookURL(book)
            }}
          />
          <div className="book-shelf-changer">
            <select
              onChange={ev => this.updateBook(book, ev.target.value)}
              defaultValue={book.shelf}
            >
              <option value="move" disabled>
                Move to...
              </option>
              <option value={BooksAPI.SHELF_CURRENTLY_READING}>
                Currently Reading
              </option>
